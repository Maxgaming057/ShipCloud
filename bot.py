...
async def create_server_task(interaction):
    await interaction.response.send_message(embed=discord.Embed(description="🛠️ **Creating Vps Your,Powered by [ShipClouds](https://discord.gg/VUVmZ3cS)**", color=0x00ff00))
    userid = str(interaction.user.id)
    ...
    try:
        container_id = subprocess.check_output([
           "docker", "run", "-itd", "--privileged", "--hostname", "ShipClouds", "--cap-add=ALL", image
        ]).strip().decode('utf-8')
    except subprocess.CalledProcessError as e:
